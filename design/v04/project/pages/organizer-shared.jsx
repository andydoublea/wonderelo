// Wonderelo — Phase 03/04 shared atoms (organizer + account + public event)
// Exposes everything via window so screen-*.jsx files can pull them in.
...
// ── Billing/plan state (mocked) ──────────────────────────────
// Mirrors the real model: a user is on an active SUBSCRIPTION (capacity tier),
// or has prepaid single-event CREDITS, or is on the FREE tier. Server checks
// subscription first, then credits. Dev toggles in the Overview flip this via
// window.setBilling('subscription' | 'credits' | 'free').
if (typeof window !== 'undefined') {
  window.__wBilling = window.__wBilling || 'subscription';
  window.setBilling = (v) => { window.__wBilling = v; window.dispatchEvent(new CustomEvent('w-billing')); };
}
...
const Logo = ({ size = 120, wordmark = 32, dark = false }) => (
...
const Btn = ({ children, variant = 'primary', size = 'md', full = false, leadingIcon, trailingIcon, style = {} }) => {
  ...
  const sz = size === 'sm' ? { padding: '8px 12px', fontSize: 13 }
...
function OrgNav({ active = 'Rounds' }) {
  const tabs = ['Rounds', 'Event page'];
  const [menuOpen, setMenuOpen] = React.useState(false);
  const [billing, setBillingState] = React.useState((typeof window !== 'undefined' && window.__wBilling) || 'subscription');
  React.useEffect(() => {
    const h = () => setBillingState(window.__wBilling || 'subscription');
  ...
  const MenuItem = ({ icon, children, danger, active: isActive }) => (
...
function FormField({ label, value = '', placeholder, hint, prefix, suffix, focused = false, helper, width, type = 'text' }) {
  return (
...
// DEV-only segmented control. Flips the mocked billing state shown in the nav
// pill (subscription / prepaid credits / free) so all variants are reviewable
// on the page itself. Not part of the production UI.
function PlanDevToggle() {
  const [billing, setLocal] = React.useState((typeof window !== 'undefined' && window.__wBilling) || 'subscription');
  React.useEffect(() => {
    const h = () => setLocal(window.__wBilling || 'subscription');
    window.addEventListener('w-billing', h);
    return () => window.removeEventListener('w-billing', h);
  }, []);
  const opts = [
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: C.fontMono, fontSize: 10, fontWeight: 700, letterSpacing: '.16em', textTransform: 'uppercase', color: C.ink, opacity: .45 }}>
        <span style={{ width: 5, height: 5, borderRadius: '50%', background: C.orange }} />Dev · plan
      </span>
      <div style={{ display: 'inline-flex', padding: 3, gap: 2, borderRadius: 9, border: `1px dashed ${C.hairStrong}`, background: 'rgba(255,255,255,.5)' }}>
        {opts.map(o => {
          const on = o.v === billing;
          return (
            <button key={o.v} onClick={() => window.setBilling(o.v)} style={{
              padding: '5px 11px', borderRadius: 6, border: 'none', cursor: 'pointer',
              fontFamily: C.fontBody, fontSize: 12, fontWeight: 600,
              background: on ? C.purpleDeep : 'transparent',
              color: on ? '#fff' : C.ink, opacity: on ? 1 : .6,
            }}>{o.l}</button>
          );
        })}
      </div>
...
Object.assign(window, {
  WC: C, Italic, Diamond, Eyebrow, Logo, Btn, ArrowR,
  OrgNav, FormField, PageShell, PageHead, PlanDevToggle, StudioFooter,
});
