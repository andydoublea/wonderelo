...
- **Version reporting.** After EVERY deploy (staging or production), report the full version string back to the user in chat — git short SHA + build time, e.g. `b2d830b · 260502-1545`. The user verifies the deploy by reading the version badge bottom-left (green) on the live site. If the badge doesn't match what was reported, the deploy hasn't propagated or the user needs a hard refresh (Cmd+Shift+R). The git hash comes from the commit Vercel builds (`VERCEL_GIT_COMMIT_SHA`) — for a `main` deploy via PR merge, it's the merge commit on `main`, not the source commit. `APP_VERSION` in `src/utils/version.tsx` is legacy and no longer shown in the badge — don't bother bumping it.
