...
const fallbackPosts: Record<string, BlogPost> = {
  '5-networking-tips': {
    id: '1', slug: '5-networking-tips', category: 'Networking',
  ...
  'speed-dating-format': {
    id: '2', slug: 'speed-dating-format', category: 'Event design',
  ...
  'hybrid-events': {
    id: '3', slug: 'hybrid-events', category: 'Hybrid',
  ...
  'random-vs-ai-matching': {
    id: '4', slug: 'random-vs-ai-matching', category: 'Research',
  ...
  'how-to-promote-your-event': {
    id: '5', slug: 'how-to-promote-your-event', category: 'Networking',
  ...
  'meeting-points-guide': {
    id: '6', slug: 'meeting-points-guide', category: 'Networking',
    title: 'Meeting points: the secret ingredient of great networking rounds',
    excerpt: 'Why designated meeting spots make networking less awkward, more efficient, and way more fun for everyone involved — with practical setups from three real events.',
    content: `<h2>What are <em>meeting points?</em></h2><p>Meeting points are designated spots where matched participants meet for their networking conversation. Instead of wandering around looking for each other, both people know exactly where to go — <strong>Table 3, the Corner Lounge, or the Blue Flag area.</strong></p><p>The idea sounds almost too simple to write down. And yet, of all the things we've changed about networking rounds over the past two years, this is the one organizers tell us made the biggest difference.</p><blockquote class="bp-pull">"We thought the matching algorithm was the magic. Turns out the real magic was telling people exactly where to stand."<span class="who"><strong>Marek H.</strong> — co-organizer, Forum 2025</span></blockquote><h2>Why they matter more than <em>you think</em></h2><p>Without meeting points, matched participants waste precious minutes finding each other. In a crowded venue, this can eat up half the round time. With clear meeting points, conversations start immediately — and that compounds.</p><p>We measured this at three events of similar size:</p><div class="bp-figure"><div class="big">+40<em>%</em></div><div class="body"><strong>Longer conversations.</strong> Events with clearly-named meeting points saw average per-pair talk time grow from 4 min 12 s to 5 min 54 s on a six-minute round.</div></div><h2>How to <em>set them up</em></h2><p>Choose locations that are:</p><ul><li><strong>Easy to find</strong> — numbered tables, colored zones, or recognizable landmarks (the espresso bar, the merch wall).</li><li><strong>Spread across the venue</strong> — avoid clustering everyone in one area; you'll create a bottleneck.</li><li><strong>Conversation-friendly</strong> — away from loud speakers, the stage feed, and the main flow of foot traffic.</li></ul><p>Most organizers get this right on the second event, never the first. If you only do one thing the first time around, walk the venue an hour before doors with the points marked on your phone and check the noise from each one.</p><h2>Creative <em>meeting-point</em> ideas</h2><p>Go beyond numbered tables. Use themed areas — <em>"The Innovation Corner"</em>, <em>"The Wonderelo Lounge"</em> — or even outdoor spaces for a fresh-air networking experience. At one wedding we set up four meeting points named after the couple's favorite films. Guests posted photos with the signs all night.</p><p>The name does double duty: it tells participants where to go, and it tells them what kind of conversation they're walking into.</p><h2>The <em>result</em></h2><p>Events with well-designed meeting points see longer conversations, higher follow-up rates, and — based on the post-event survey — noticeably higher reported enjoyment. When people don't waste time finding each other, they spend more time connecting. And that's what networking is all about.</p>`,
    imageUrl: 'https://images.unsplash.com/photo-1517457373958-b7bdd4587205?w=800&h=400&fit=crop',
    readTime: '5 min read',
    createdAt: '2026-01-30T00:00:00Z', updatedAt: '2026-01-30T00:00:00Z',
  },
};
...
export function BlogDetailPage() {
  ...
  const fetchRelatedPosts = async (currentSlug: string) => {
    // Design "Keep reading" set (Blog Post.html): promote → random-vs-ai → 5-tips.
    // Curate that order for the fallback, filtering the current post and topping
    // up from the remaining sample posts so the grid always has three cards.
    const pickFallback = () => {
      const preferred = ['how-to-promote-your-event', 'random-vs-ai-matching', '5-networking-tips'];
      const ordered = [
        ...preferred,
        ...Object.keys(fallbackPosts).filter((s) => !preferred.includes(s)),
      ];
      return ordered
        .filter((s) => s !== currentSlug)
        .map((s) => fallbackPosts[s])
        .filter(Boolean)
        .slice(0, 3);
    };
    try {
      ...
      <main className="w-shell">
        ...
        <header className="bp-header">
          ...
          <div className="bp-byline">
            ...
            <div className="who">
              <span className="name">Andy K.</span>
        ...
        <section className="bp-grid">
          ...
          <article className="bp-body">
            ...
            {/* Author signature — design `.bp-author` = avatar + handwritten
                sig-name "Andy Abel" + `.who` line. The shared <AuthorSignature/>
                (LinkedIn icon, "Founder & CEO", <hr>, 49-countries bio) is
                intentionally NOT rendered to match the mock 1:1; its code is
                preserved in ./AuthorSignature.tsx. */}
            ...
            {relatedPosts.map((post, i) => (
              ...
              >
                ...
                <div className="body">
                  <h3>{post.title}</h3>
                  <div className="meta">{(() => {
                    const cat = post.category || post.tags?.[0] || 'Networking';
                    return post.readTime ? `${cat} · ${post.readTime}` : cat;
                  })()}</div>
                </div>
              </a>
            ))}
        ...
        <section className="bp-cta">
          ...
          <div>
            <h2>Try meeting points at <span className="w-italic">your</span> next event</h2>
